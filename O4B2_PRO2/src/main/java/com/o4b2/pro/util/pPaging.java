...
public class pPaging {
	...
	//생성자로 초기 필수 데이터 저장
	public pPaging(int maxNum, int pageNum, int listCnt, 
	...
	public String makePaging(int hoscheck) {
		//전체 페이지 개수
		//게시글이 9개 이하 - 1
		//게시글이 11개 이상 - 2
		...
		//'이전' 버튼 처리
		if(start != 1) {
			sb.append("<a class='pno' href='" + "prescription" +
					"?check=" + hoscheck +"&pageNum=" + (start - 1) + "'>");
			sb.append("&nbsp;이전&nbsp;");
			sb.append("</a>");
